package com.jin.demo.springmvc.servlet;


import com.jin.demo.springmvc.annotation.*;
import com.jin.demo.springmvc.MyHandler;
...
public class MyDispatcherServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 5 构造一个HandlerMapping处理器映射器，将配置好的url和Method建立映射关系，并且设置访问权限
        initHandlerMapping();

        log.info("My-SpringMVC 初始化完成....");
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String, Object> entry: ioc.entrySet()) {
            ...
            if(!aClass.isAnnotationPresent(MyRestController.class)) {
                continue;
            }
            String baseUrl = "";
            if(aClass.isAnnotationPresent(MyRestController.class)) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                MyHandler handler = new MyHandler(entry.getValue(),method, Pattern.compile(url));
                // 权限控制
                securityManage(security,method,handler,url);
     ...
     * @param url
     */
    private void securityManage(String[] security, Method method, MyHandler handler, String url) {
        if (method.isAnnotationPresent(MySecurity.class)){
            String[] securityMethod = method.getAnnotation(MySecurity.class).value();
            // 如果方法上有@MySecurity注解，就以方法上的权限为准
            if (ArrayUtils.isNotEmpty(securityMethod)){
                Arrays.stream(securityMethod).forEach(s -> handler.getUsers().add(s));
                return;
            }
        }
        // 如果方法上没有@MySecurity注解，就以类上面的权限为准
        if (ArrayUtils.isNotEmpty(security)){
            Arrays.stream(security).forEach(s -> handler.getUsers().add(s));
        }else {
            // 如果方法上和类上面都没有@MySecurity注解，表示该资源不能被任何人访问
            log.error("url: {} 该资源不能被任何人访问",url);
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        for(Map.Entry<String, String[]> param: parameterMap.entrySet()) {
            ...
            // 如果没有权限，直接返回
            if (!handler.getUsers().contains(value)){
                resp.getWriter().write("Permission denied, You need to consult the administrator.");
                return;
            }
        }
